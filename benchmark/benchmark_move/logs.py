...
from benchmark_move.utils import Print
...
class LogParser:
    def __init__(self, clients, nodes, faults):
        ...
        self.size, self.rate, self.start, self.end, misses, self.sent_samples, self.received_samples, commits, self.burst \
            = zip(*results)
    ...
    def _parse_clients(self, log):
        ...
        size = 184 # Same as hotcrypto for comparison
        rate = int(search(r'Transactions rate: (\d+)', log).group(1))
        burst = int(search(r'Burst: (\d+)', log).group(1))

        tmp = search(r'\[(.*Z) .* Start sending transactions', log).group(1)
        start = self._to_posix(tmp)

        tmp = findall(r'\[(.*Z) .* Stop sending transactions', log)

        tmp += findall(r'\[(.*Z) .* Stop processing transactions', log)
        tmp = max(tmp)
        ...
        tmp = findall(r'\[(.*Z) .* Processed sample transaction (\d+)', log)
        #     tmp = findall(r'Batch ([^ ]+) contains sample tx (\d+)', log)
        received_samples = {int(s): self._to_posix(ts) for ts, s in tmp}

        tmp = findall(r'\[(.*Z) .* Processed sample transaction (\d+)', log)
        tmp = [(d, self._to_posix(t)) for t, d in tmp]
        commits = self._merge_results([tmp])

        return size, rate, start, end, misses, samples, received_samples, commits, burst
    ...
    def _diem_throughput(self):
        if not self.commits:
            print("Nothing in commits")
            return 0, 0, 0
        # start, end = min(self.start), max(self.commits.values())
        # duration = end - start
        # //TODOTODO Could use the nonce of the last sample transaction as an approximation
        nb_tx = self.burst[0] * (len(self.commits.values()) - 1)
        # print("nb tx for thoughput calculation:", nb_tx)
        # last_tx = max(self.commits.values())
        # start, end = min(self.start), last_tx
        start, end = min(self.start), max(self.end)
        ...
        # tps = len(self.commits.values()) / duration
        # bps = tps * self.size[0]
        return tps, bps, duration

    def _diem_latency(self):
        latency = []
        for sent, received in zip(self.sent_samples, self.received_samples):
            ...
            for sample, ts in received.items():
                ...
                if sample in sent:
                    ...
                    latency += [end-start]
        # print("Average latency of", len(latency), "tx is", mean(latency))
        return mean(latency) if latency else 0
    ...
    def result(self):
        ...
        diem_tps, diem_bps, duration = self._diem_throughput()
        diem_latency = self._diem_latency() * 1000
        ...
        return (
            ...
            # '\n'
            f' Diem TPS: {round(diem_tps):,} tx/s\n'
            f' Diem BPS: {round(diem_bps):,} B/s\n'
            f' Diem latency: {round(diem_latency):,} ms\n'
            '-----------------------------------------\n'
        )
