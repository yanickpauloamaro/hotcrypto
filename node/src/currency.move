...
module Currency::BasicCoin {
    use Std::Errors;
    use Std::Signer;

    /// Address of the owner of this module
    const MODULE_OWNER: address = @Currency;

    /// Error codes
    const ENOT_MODULE_OWNER: u64 = 0;
    const EINSUFFICIENT_BALANCE: u64 = 1;
    const EALREADY_HAS_BALANCE: u64 = 2;
    const EDOESNT_HAVE_BALANCE: u64 = 3;
    ...
    public fun balance_of(owner: address): u64 acquires Balance {
        assert!(exists<Balance>(owner), Errors::already_published(EDOESNT_HAVE_BALANCE));
        borrow_global<Balance>(owner).coin.value
    }

    /// Transfers `amount` of tokens from `from` to `to`.
    public fun transfer(from: &signer, to: address, amount: u64) acquires Balance {
        assert!(exists<Balance>(Signer::address_of(from)), Errors::already_published(EDOESNT_HAVE_BALANCE));
        assert!(exists<Balance>(to), Errors::already_published(EDOESNT_HAVE_BALANCE));
